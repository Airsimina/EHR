<template>
  <div class="leaveApply">
    ...
    <div class="wrap-1">
      ...
      <!-- 流程区域 -->
      <div class="flow-box">
        <div class="flow-item" v-for="(item, index) in flowHiComments" :key="index">
          <!-- 销售计划 左侧时间线 -->
          <div :class="[getStatus(item.operateType)]" class="state"></div>
          <div class="flow-info">
            <div class="name-box">
              <div class="flow-name">{{item.operaterName}}</div>
              <div class="position">{{item.nodeName}}</div>
            </div>
            <div class="approval-info" v-if="item.operateType=='驳回'">意见：{{item.opinion}}</div>
            <div class="time">{{item.operateTime}}</div>
            <!-- 销售计划 右侧展示文字 -->
            <div :class="[getStatus(item.operateType)]" class="status">{{item.operateType}}</div>
          </div>
        </div>
      </div>
      <!-- 底部按钮 -->
      <div class="footer-box" v-if="editFlag=='1' || removeFlag=='1'">
        <div class="btn edit-btn" v-show="editFlag!='2'">
          <span @click="editFun('1')" class="btn xj-btn" :class="{'forbid':editFlag=='2'}">修改</span>
        </div>
        <div class="btn xj-btn" v-show="removeFlag!='2'" :class="{'forbid':removeFlag=='2'}">
